        ...
        <form action="{{ route('admin.transactions.update', $transaction->id) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="patron_name" value="{{ old('patron_name', $transaction->patron_name) }}"
                ...
                <div>
                    ...
                    <select name="type"
                        ...
                        <option value="directional"    {{ old('type', $transaction->type) === 'directional'    ? 'selected' : '' }}>Directional</option>
                        <option value="informational" {{ old('type', $transaction->type) === 'informational' ? 'selected' : '' }}>Informational</option>
                        <option value="research"       {{ old('type', $transaction->type) === 'research'       ? 'selected' : '' }}>Research</option>
                        <option value="reader_advisory"{{ old('type', $transaction->type) === 'reader_advisory'? 'selected' : '' }}>Reader Advisory</option>
                        <option value="technology"     {{ old('type', $transaction->type) === 'technology'     ? 'selected' : '' }}>Technology</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="duration" value="{{ old('duration', $transaction->duration) }}" min="1"
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="question" rows="3"
                              ...
                              required>{{ old('question', $transaction->question) }}</textarea>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="answer" rows="3"
                              class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500">{{ old('answer', $transaction->answer) }}</textarea>
                </div>

                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Additional Notes</label>
                    <textarea name="notes" rows="2"
                              class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500">{{ old('notes', $transaction->notes) }}</textarea>
                </div>
